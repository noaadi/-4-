...
export const LoginScreen = () => {
  ...
  // Existing user → skip questionnaire, go straight to main
  const handleLogin = () => {
    navigate('/main');
  };

  // New user → go through questionnaire first
  const handleRegister = () => {
    navigate('/questionnaire/gender');
  };

  return (
    <MobileContainer className="p-5 justify-start pt-10 space-y-4 bg-pink-50 overflow-y-auto">
      <div className="flex flex-col items-center justify-center space-y-1">
        <Character state={characterState} size="md" />
        <h1 className="text-2xl font-bold text-gray-800">LAZOZ</h1>
      </div>
      ...
      <div className="space-y-3 w-full">
        ...
        <Button variant="secondary" onClick={handleRegister} fullWidth>{t('createAccount')}</Button>
      </div>
